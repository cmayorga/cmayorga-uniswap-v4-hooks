...
contract TestPointsHook is Test, Deployers, ERC1155TokenReceiver {
    ...
    function test_swap() public {
        ...
        //assertEq(hook.balanceOf(address(this), 0), 0.0002 ether, 'Invalid user points balance');
        //assertEq(hook.balanceOf(address(this), 1), 0.0002 ether, 'Invalid user points balance');
        //assertEq(hook.balanceOf(address(this), 2), 0.0002 ether, 'Invalid user points balance');
        assertEq(pointsBalanceAfterSwap - pointsBalanceOriginal, 2 * 10 ** 14);
        assertGt(token.balanceOf(address(this)), 0, 'User should have TOKEN');
    }    
}